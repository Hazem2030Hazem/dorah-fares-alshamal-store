...
// ============================================================
// 🤖 DORA SMART RECOMMENDATIONS SYSTEM
// ============================================================
let recentlyViewed = JSON.parse(localStorage.getItem('doraRecentlyViewed')) || [];
const MAX_RECENT = 8;

function addToRecentlyViewed(id){ recentlyViewed=recentlyViewed.filter(function(x){return x!==id;}); recentlyViewed.unshift(id); if(recentlyViewed.length>MAX_RECENT) recentlyViewed.pop(); localStorage.setItem('doraRecentlyViewed',JSON.stringify(recentlyViewed)); }
(function(){ var orig=openProductModal; openProductModal=function(id){ addToRecentlyViewed(id); orig(id); }; var orig2=openQuickView; openQuickView=function(id){ addToRecentlyViewed(id); orig2(id); }; })();

function getBestSellers(limit){ return (productsData||[]).filter(function(p){return p.stock>0;}).sort(function(a,b){return (b.rating||0)-(a.rating||0);}).slice(0,limit||8); }

function getFBT(productId){ var p=productsData.find(function(x){return x.id===productId;}); if(!p) return[]; return productsData.filter(function(x){return x.category===p.category&&x.id!==productId&&x.stock>0;}).sort(function(a,b){return (b.rating||0)-(a.rating||0);}).slice(0,3); }

function getSimilarProducts(productId,limit){ var p=productsData.find(function(x){return x.id===productId;}); if(!p) return[]; var same=productsData.filter(function(x){return x.category===p.category&&x.id!==productId&&x.stock>0;}); var comp={printers:['ink','cables'],computers:['ram','storage','accessories'],ram:['computers','storage'],storage:['computers','ram','cables'],projectors:['cables','accessories'],accessories:['computers','cables'],ink:['printers'],cables:['projectors','computers','printers'],food:['food']}; var extra=[]; if(comp[p.category]){ extra=productsData.filter(function(x){return comp[p.category].indexOf(x.category)!==-1&&x.id!==productId&&x.stock>0;}); } return same.concat(extra).slice(0,limit||4); }

(function(){ var orig=openProductModal; openProductModal=function(id){ orig(id); setTimeout(function(){ var info=document.querySelector('#productModal .modal-info'); if(info&&!info.querySelector('.dora-fbt')){ var fbt=getFBT(id); if(fbt.length){ var h='<div class="dora-fbt" style="margin-top:20px;padding:20px;background:rgba(59,130,246,0.05);border-radius:16px;border:1px solid rgba(59,130,246,0.2)"><h4 style="margin:0 0 15px;font-size:16px">🎯 يشتري العملاء معاً غالباً</h4><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:10px">'; fbt.forEach(function(p){ h+='<div onclick="openProductModal('+p.id+')" style="cursor:pointer;background:white;border-radius:12px;padding:12px;text-align:center;border:1px solid rgba(0,0,0,0.1)"><img src="'+p.image+'" style="width:70px;height:70px;object-fit:cover;border-radius:8px;margin-bottom:6px" loading="lazy"><div style="font-size:11px;font-weight:bold;color:#1F2937">'+(p.name||'').substring(0,20)+'...</div><div style="font-size:13px;color:#3B82F6;font-weight:bold">'+(typeof formatPrice==='function'?formatPrice(p.price):p.price)+'</div></div>'; }); h+='</div></div>'; var rel=document.getElementById('relatedProducts'); if(rel) rel.insertAdjacentHTML('beforebegin',h); else info.insertAdjacentHTML('beforeend',h); } } },300); }; })();

function renderRecentlyViewed(){ var g=document.getElementById('recentlyViewedGrid'); var s=document.getElementById('recentlyViewedSection'); if(!g) return; var ids=recentlyViewed.filter(function(id){return productsData.find(function(p){return p.id===id;});}); if(!ids.length){ if(s) s.style.display='none'; return; } if(s) s.style.display='block'; g.innerHTML=ids.map(function(id){ var p=productsData.find(function(x){return x.id===id;}); if(!p) return''; var stars='★'.repeat(Math.floor(p.rating||0))+'☆'.repeat(5-Math.floor(p.rating||0)); var has=p.oldPrice&&p.oldPrice>p.price; return'<div class="prod-card" onclick="openProductModal('+p.id+')" style="cursor:pointer"><div class="prod-img" style="height:160px">'+(p.badge?'<div class="prod-badge">'+p.badge+'</div>':'')+(has&&!p.badge?'<div class="prod-badge discount">خصم</div>':'')+'<img src="'+p.image+'" loading="lazy" style="height:100%;width:100%;object-fit:cover"></div><div class="prod-body" style="padding:10px"><span class="prod-tag" style="font-size:10px">'+(catLabels[p.category]||p.category)+'</span><h4 style="font-size:12px;margin:4px 0">'+(p.name||'')+'</h4><div style="font-size:11px;color:#FFD700">'+stars+'</div><div style="font-size:14px;font-weight:bold">'+(typeof formatPrice==='function'?formatPrice(p.price):p.price)+'</div></div></div>'; }).join(''); }

function renderBestSellers(){ var g=document.getElementById('bestSellersGrid'); if(!g) return; var bs=getBestSellers(8); g.innerHTML=bs.map(function(p,i){ var stars='★'.repeat(Math.floor(p.rating||0))+'☆'.repeat(5-Math.floor(p.rating||0)); var has=p.oldPrice&&p.oldPrice>p.price; var medal=i===0?'🥇':i===1?'🥈':i===2?'🥉':''; return'<div class="prod-card" onclick="openProductModal('+p.id+')" style="cursor:pointer;position:relative">'+(medal?'<div style="position:absolute;top:8px;left:8px;font-size:28px;z-index:10">'+medal+'</div>':'')+'<div class="prod-img" style="height:160px">'+(p.badge?'<div class="prod-badge">'+p.badge+'</div>':'')+(has&&!p.badge?'<div class="prod-badge discount">خصم</div>':'')+'<img src="'+p.image+'" loading="lazy" style="height:100%;width:100%;object-fit:cover"></div><div class="prod-body" style="padding:10px"><span class="prod-tag" style="font-size:10px">'+(catLabels[p.category]||p.category)+'</span><h4 style="font-size:12px;margin:4px 0">'+(p.name||'')+'</h4><div style="font-size:11px;color:#FFD700">'+stars+' <span style="color:#6B7280">'+(p.rating||0)+'</span></div><div style="font-size:14px;font-weight:bold">'+(typeof formatPrice==='function'?formatPrice(p.price):p.price)+'</div></div></div>'; }).join(''); }

function injectRecommendationSections(){ if(document.getElementById('recentlyViewedSection')) return; var ps=document.getElementById('products'); if(!ps) return; var h='<section id="recentlyViewedSection" style="display:none;padding:60px 0;background:rgba(59,130,246,0.02)"><div class="container"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:30px"><h2 style="font-size:28px">👁️ شاهدتها مؤخراً</h2><button onclick="localStorage.removeItem(\'doraRecentlyViewed\');recentlyViewed=[];renderRecentlyViewed();" style="background:none;border:1px solid rgba(0,0,0,0.2);padding:8px 16px;border-radius:8px;cursor:pointer;font-size:13px">🗑️ مسح السجل</button></div><div id="recentlyViewedGrid" class="prod-grid" style="grid-template-columns:repeat(auto-fill, minmax(180px, 1fr));gap:16px"></div></div></section><section id="bestSellersSection" style="padding:60px 0"><div class="container"><h2 style="font-size:28px;margin-bottom:30px">🔥 الأكثر مبيعاً</h2><div id="bestSellersGrid" class="prod-grid" style="grid-template-columns:repeat(auto-fill, minmax(180px, 1fr));gap:16px"></div></div></section>'; ps.insertAdjacentHTML('beforebegin',h); }

(function(){ if(document.getElementById('dora-rec-styles')) return; var s=document.createElement('style'); s.id='dora-rec-styles'; s.textContent='@media (max-width:768px){#recentlyViewedGrid,#bestSellersGrid{grid-template-columns:repeat(2,1fr)!important;gap:10px!important}}.dora-fbt{animation:doraFadeIn .4s ease}@keyframes doraFadeIn{from{opacity:0;transform:translateY(20px)}to{opacity:1;transform:translateY(0)}}'; document.head.appendChild(s); })();

document.addEventListener('DOMContentLoaded',function(){ setTimeout(function(){ injectRecommendationSections(); renderRecentlyViewed(); renderBestSellers(); },600); });
})();
